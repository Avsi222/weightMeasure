...
class NinthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "TenthSB", bundle: nil).instantiateViewController(withIdentifier: "TenthVC") as? TenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension NinthVC: BaseViewProtocol {
