...
class FifteenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "SixteenthSB", bundle: nil).instantiateViewController(withIdentifier: "SixteenthVC") as? SixteenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension FifteenthVC: BaseViewProtocol {
