...
class NineteenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "TwentiethSB", bundle: nil).instantiateViewController(withIdentifier: "TwentiethVC") as? TwentiethVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension NineteenthVC: BaseViewProtocol {
