...
class TwelveVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "ThirteSB", bundle: nil).instantiateViewController(withIdentifier: "ThirteVC") as? ThirteVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension TwelveVC: BaseViewProtocol {
