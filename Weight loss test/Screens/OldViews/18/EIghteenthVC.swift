...
class EIghteenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "NineteenthSB", bundle: nil).instantiateViewController(withIdentifier: "NineteenthVC") as? NineteenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension EIghteenthVC: BaseViewProtocol {
