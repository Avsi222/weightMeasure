...
class TenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "EleventhSB", bundle: nil).instantiateViewController(withIdentifier: "EleventhVC") as? EleventhVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension TenthVC: BaseViewProtocol {
