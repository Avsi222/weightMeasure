...
class EleventhVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "TwelveSB", bundle: nil).instantiateViewController(withIdentifier: "TwelveVC") as? TwelveVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension EleventhVC: BaseViewProtocol {
