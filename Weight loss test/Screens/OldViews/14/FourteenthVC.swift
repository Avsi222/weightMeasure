...
class FourteenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "FifteenthSB", bundle: nil).instantiateViewController(withIdentifier: "FifteenthVC") as? FifteenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension FourteenthVC: BaseViewProtocol {
