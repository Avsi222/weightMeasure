...
class ThirteVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "FourteenthSB", bundle: nil).instantiateViewController(withIdentifier: "FourteenthVC") as? FourteenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension ThirteVC: BaseViewProtocol {
