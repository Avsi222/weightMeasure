...
class SeventeenthVC: UIViewController {
    ...
    @IBAction private func nextButtonIsClicked(_ sender: UIButton) {
        guard let vc = UIStoryboard.init(name: "EIghteenthSB", bundle: nil).instantiateViewController(withIdentifier: "EIghteenthVC") as? EIghteenthVC else {fatalError()}
        navigationController?.pushViewController(vc, animated: true)
    }
    
}

extension SeventeenthVC: BaseViewProtocol {
